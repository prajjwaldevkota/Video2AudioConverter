...
import { Search, Download, Music, Settings, Sparkles, Zap, Crown, Radio, Info } from "lucide-react";
...
export default function App() {
  ...
  const search = async () => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch(`${URL}/search?query=${encodeURIComponent(userInput)}`);
      if (!response.ok) {
  ...
  const handleDownload = async (videoUrl, videoTitle) => {
    ...
    try {
      const response = await fetch(`${URL}/download?url=${encodeURIComponent(
        videoUrl
  ...
  const getMethodInfo = (selectedMethod) => {
    const methodInfo = {
      auto: { icon: Sparkles, label: "Auto", description: "Smart selection based on format", color: "purple" },
      fast: { icon: Zap, label: "Fast", description: "Quick processing with yt-dlp", color: "blue" },
      quality: { icon: Crown, label: "Quality", description: "Best quality with FFmpeg", color: "yellow" },
      stream: { icon: Radio, label: "Stream", description: "Instant download start", color: "green" }
    };
  ...
  const getFormatInfo = (selectedFormat) => {
    const formatInfo = {
      ...
      wav: { quality: "Lossless", compatibility: "Universal", size: "Very Large" },
        ...
        <div className="w-full max-w-6xl bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl p-8 space-y-8 mb-12">
          ...
          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center mb-6">
              ...
              <h3 className="text-white font-semibold">Advanced Audio Settings</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                ...
                >
                  <option value="mp3" className="bg-gray-800">MP3 - Universal</option>
                  <option value="aac" className="bg-gray-800">AAC - High Quality</option>
                  <option value="alac" className="bg-gray-800">ALAC - Apple Lossless</option>
                  <option value="flac" className="bg-gray-800">FLAC - Free Lossless</option>
                  <option value="wav" className="bg-gray-800">WAV - Uncompressed</option>
                  <option value="ogg" className="bg-gray-800">OGG - Open Source</option>
                </select>
                <div className="mt-2 text-xs text-white/50">
                  Quality: {getFormatInfo(format).quality} • Size: {getFormatInfo(format).size}
                </div>
              </div>
                ...
                >
                  {["128", "192", "256", "320"].map((b) => (
                    <option key={b} value={b} className="bg-gray-800">
                      {b} kbps {b === "320" ? "(Highest)" : b === "128" ? "(Fastest)" : ""}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  <option value="auto" className="bg-gray-800">Auto - Smart Selection</option>
                  <option value="fast" className="bg-gray-800">Fast - Quick Process</option>
                  <option value="quality" className="bg-gray-800">Quality - Best Audio</option>
                  <option value="stream" className="bg-gray-800">Stream - Instant Start</option>
                </select>
              ...
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="flex items-center mb-2">
                  <MethodIcon className={`w-4 h-4 mr-2 text-${methodConfig.color}-400`} />
                  <span className="text-white text-sm font-medium">{methodConfig.label} Mode</span>
                </div>
                ...
                <div className="mt-2 flex items-center text-xs text-white/40">
                  ...
                  {method === "auto" ? "Optimized for your format" : 
                   method === "fast" ? "~30% faster processing" :
                   method === "quality" ? "Professional grade output" :
        ...
        {videos.length > 0 && (
          <div className="w-full max-w-none px-4">
            <h2 className="text-2xl font-bold text-white mb-6 text-center">
              Choose your tracks • {format.toUpperCase()} • {methodConfig.label} Mode
            </h2>
